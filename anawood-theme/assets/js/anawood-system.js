/* ══════════════════════════════════════
   ...
   Theme toggle, Cart state, Animations
   ══════════════════════════════════════ */
...
(function() {
  ...
  window.renderCartPage = function() {
    ...
    if (cart.length === 0) {
      el.innerHTML = '<div style="text-align:center;padding:48px 24px;"><p style="font-size:48px;margin-bottom:12px">🛒</p><h3>Giỏ hàng trống</h3><p style="color:var(--muted);margin-bottom:16px">Hãy thêm sản phẩm yêu thích nhé!</p></div>';
    ...
    el.innerHTML = cart.map(item => {
      ...
      return '<div class="item" data-id="' + item.id + '">' +
        ...
        '<div class="item-meta">' + (item.meta || '') + '</div></div>' +
        '<div style="text-align:right"><div class="qty" style="justify-content:flex-end">' +
        '<button onclick="updateCartQty(\'' + item.id + '\',-1)">−</button>' +
        '<strong style="min-width:22px;text-align:center">' + (item.qty || 1) + '</strong>' +
        '<button onclick="updateCartQty(\'' + item.id + '\',1)">+</button>' +
        '<span class="remove-btn" onclick="removeFromCart(\'' + item.id + '\')" title="Xóa">✕</span></div>' +
        '<div style="font-weight:700;color:#b45309;margin-top:4px">' + fmt(price * (item.qty || 1)) + '</div></div></div>';
    }).join('');
  ...
  /* Re-init animations for WooCommerce AJAX events */
  document.addEventListener('wc_fragments_refreshed', initAnimations);
  document.addEventListener('wc_fragments_loaded', initAnimations);
